        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                try
                {
                    ...
                    RV_gl.LocalReport.ReportPath = Server.MapPath("~/Reports/Fin/generalledger.rdlc");
                    RV_gl.LocalReport.DataSources.Clear();
                    RV_gl.KeepSessionAlive = false;
                    RV_gl.AsyncRendering = false;
                    RV_gl.SizeToReportContent = false;
                    ...
                    List<tt_gl_trans> GeneralLedger = _FinanceReportLL.getGeneralLedgerTransactionDtls(prp,false);
                    ...
                    //GeneralLedger.ForEach(x => x.acc_cd_desc = x.acc_cd.ToString() + "-" + " Hi");

                   dataSet = Extension.ToDataSet(GeneralLedger);
                    ReportDataSource rdc = new ReportDataSource("generalledger", dataSet.Tables[0]);
                    ...
                    RV_gl.LocalReport.SetParameters(paramss);
                    RV_gl.LocalReport.DataSources.Add(rdc);
                    RV_gl.LocalReport.Refresh();
