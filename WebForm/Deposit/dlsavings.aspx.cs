...
{
    public partial class dlsavings : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                try
                {
                    ...
                    RV_DLS.LocalReport.ReportPath = Server.MapPath("~/Reports/Deposit/dlsavings.rdlc");
                    RV_DLS.LocalReport.DataSources.Clear();
                    RV_DLS.KeepSessionAlive = true;
                    RV_DLS.AsyncRendering = true;
                    ...
                    List<tt_sbca_dtl_list> depositdetails = _DepositLL.PopulateDLSavings(prp);
                    ...
                    ReportDataSource rdc = new ReportDataSource("dlsaving", dataSet.Tables[0]);
                    ...
                    RV_DLS.LocalReport.SetParameters(paramss);
                    RV_DLS.LocalReport.DataSources.Add(rdc);
                    RV_DLS.LocalReport.Refresh();


                }
                    else
                {
                        RV_DLS.Visible = false;
